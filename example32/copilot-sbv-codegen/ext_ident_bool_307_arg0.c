/* File: "ext_ident_bool_307_arg0.c". Automatically generated by SBV. Do not edit! */

#include "internal.h"

/* User given declarations: */
/*test 003*/
/*ACSL to write
 label "verticalCriterionConflict_part2.2" (Extf_ident_double(label "theta_dir_1" Extf_ident_double(label "thetaVert" (Extf_ident_double(label "thetaVert_dividend" (Extf_ident_double(label "thetaVert_part1" (0.0 - Extf_ident_double(label "thetaVert_part1.1" (((Extf_ident_double(label "ownPositionX" (if s0 then Ext_ownship_position_x else s1)) - Extf_ident_double(label "intPositionX" (if s0 then Ext_intruder_position_x else s2))) * (Extf_ident_double(label "ownVelocityX" (if s0 then Ext_ownship_velocity_x else s7)) - Extf_ident_double(label "intVelocityX" (if s0 then Ext_intruder_velocity_x else s3)))) + ((Extf_ident_double(label "ownPositionY" (if s0 then Ext_ownship_position_y else s4)) - Extf_ident_double(label "intPositionY" (if s0 then Ext_intruder_position_y else s5))) * (Extf_ident_double(label "ownVelocityY" (if s0 then Ext_ownship_velocity_y else s8)) - Extf_ident_double(label "intVelocityY" (if s0 then Ext_intruder_velocity_y else s6)))))))) + Extf_ident_double(label "thetaVert_part1.2" (Extf_ident_double(label "dirVert" (if Extf_ident_bool(label "dirVert_part1" (Extf_ident_double(label "dirVert_part1.1" (Extf_ident_double(label "dirVert_part1.1.1" (Extf_ident_double(label "ownPositionZ" (if s0 then Ext_ownship_position_z else s9)) - Extf_ident_double(label "intPositionZ" (if s0 then Ext_intruder_position_z else s10)))) * Extf_ident_double(label "absolute_value_splitting" signum Extf_ident_double(label "dirVert_part1.1.1" (Extf_ident_double(label "ownPositionZ" (if s0 then Ext_ownship_position_z else s9)) - Extf_ident_double(label "intPositionZ" (if s0 then Ext_intruder_position_z else s10))))))) >= Ext_minimal_vertical_separation)) then Extf_ident_double(label "dirVert_part2" (Ext_direction_parameter_vertical * Extf_ident_double(label "dirVert_part2.1" signum (Extf_ident_double(label "ownPositionZ" (if s0 then Ext_ownship_position_z else s9)) - Extf_ident_double(label "intPositionZ" (if s0 then Ext_intruder_position_z else s10)))))) else -1.0)) * Extf_ident_double(label "thetaVert_part2" Extf_sqrt(((Extf_ident_double(label "thetaVert_part2.1" (((Extf_ident_double(label "ownPositionX" (if s0 then Ext_ownship_position_x else s1)) - Extf_ident_double(label "intPositionX" (if s0 then Ext_intruder_position_x else s2))) * (Extf_ident_double(label "ownVelocityX" (if s0 then Ext_ownship_velocity_x else s7)) - Extf_ident_double(label "intVelocityX" (if s0 then Ext_intruder_velocity_x else s3)))) + ((Extf_ident_double(label "ownPositionY" (if s0 then Ext_ownship_position_y else s4)) - Extf_ident_double(label "intPositionY" (if s0 then Ext_intruder_position_y else s5))) * (Extf_ident_double(label "ownVelocityY" (if s0 then Ext_ownship_velocity_y else s8)) - Extf_ident_double(label "intVelocityY" (if s0 then Ext_intruder_velocity_y else s6)))))) * Extf_ident_double(label "thetaVert_part2.2" (((Extf_ident_double(label "ownPositionX" (if s0 then Ext_ownship_position_x else s1)) - Extf_ident_double(label "intPositionX" (if s0 then Ext_intruder_position_x else s2))) * (Extf_ident_double(label "ownVelocityX" (if s0 then Ext_ownship_velocity_x else s7)) - Extf_ident_double(label "intVelocityX" (if s0 then Ext_intruder_velocity_x else s3)))) + ((Extf_ident_double(label "ownPositionY" (if s0 then Ext_ownship_position_y else s4)) - Extf_ident_double(label "intPositionY" (if s0 then Ext_intruder_position_y else s5))) * (Extf_ident_double(label "ownVelocityY" (if s0 then Ext_ownship_velocity_y else s8)) - Extf_ident_double(label "intVelocityY" (if s0 then Ext_intruder_velocity_y else s6))))))) - Extf_ident_double(label "thetaVert_part2.3" (Extf_ident_double(label "thetaVert_part2.3.1" Extf_ident_double(label "normsq2dim" (((Extf_ident_double(label "ownVelocityX" (if s0 then Ext_ownship_velocity_x else s7)) - Extf_ident_double(label "intVelocityX" (if s0 then Ext_intruder_velocity_x else s3))) * (Extf_ident_double(label "ownVelocityX" (if s0 then Ext_ownship_velocity_x else s7)) - Extf_ident_double(label "intVelocityX" (if s0 then Ext_intruder_velocity_x else s3)))) + ((Extf_ident_double(label "ownVelocityY" (if s0 then Ext_ownship_velocity_y else s8)) - Extf_ident_double(label "intVelocityY" (if s0 then Ext_intruder_velocity_y else s6))) * (Extf_ident_double(label "ownVelocityY" (if s0 then Ext_ownship_velocity_y else s8)) - Extf_ident_double(label "intVelocityY" (if s0 then Ext_intruder_velocity_y else s6))))))) * Extf_ident_double(label "thetaVert_part2.3.2" (Extf_ident_double(label "normsq2dim" (((Extf_ident_double(label "ownPositionX" (if s0 then Ext_ownship_position_x else s1)) - Extf_ident_double(label "intPositionX" (if s0 then Ext_intruder_position_x else s2))) * (Extf_ident_double(label "ownPositionX" (if s0 then Ext_ownship_position_x else s1)) - Extf_ident_double(label "intPositionX" (if s0 then Ext_intruder_position_x else s2)))) + ((Extf_ident_double(label "ownPositionY" (if s0 then Ext_ownship_position_y else s4)) - Extf_ident_double(label "intPositionY" (if s0 then Ext_intruder_position_y else s5))) * (Extf_ident_double(label "ownPositionY" (if s0 then Ext_ownship_position_y else s4)) - Extf_ident_double(label "intPositionY" (if s0 then Ext_intruder_position_y else s5)))))) - (Ext_minimal_horizontal_separation * Ext_minimal_horizontal_separation)))))))))))) / Extf_ident_double(label "thetaVert_divisor" Extf_ident_double(label "normsq2dim" (((Extf_ident_double(label "ownVelocityX" (if s0 then Ext_ownship_velocity_x else s7)) - Extf_ident_double(label "intVelocityX" (if s0 then Ext_intruder_velocity_x else s3))) * (Extf_ident_double(label "ownVelocityX" (if s0 then Ext_ownship_velocity_x else s7)) - Extf_ident_double(label "intVelocityX" (if s0 then Ext_intruder_velocity_x else s3)))) + ((Extf_ident_double(label "ownVelocityY" (if s0 then Ext_ownship_velocity_y else s8)) - Extf_ident_double(label "intVelocityY" (if s0 then Ext_intruder_velocity_y else s6))) * (Extf_ident_double(label "ownVelocityY" (if s0 then Ext_ownship_velocity_y else s8)) - Extf_ident_double(label "intVelocityY" (if s0 then Ext_intruder_velocity_y else s6)))))))))) > 0.0)
*/
/*@
 assigns \nothing;
 ensures \result == ((((ext_ident_double_306) > (0.0))));
*/
SBool ext_ident_bool_307_arg0(const SDouble ext_ident_double_306,
                              const SDouble ext_ident_double_305,
                              const SDouble ext_ident_double_294,
                              const SDouble ext_ident_double_237,
                              const SDouble ext_ident_double_236,
                              const SDouble ext_ident_double_228, const SBool *queue_0,
                              const SWord32 ptr_0, const SDouble ext_ownship_position_x,
                              const SDouble *queue_1, const SWord32 ptr_1,
                              const SDouble ext_ident_double_229,
                              const SDouble ext_intruder_position_x, const SDouble *queue_2,
                              const SWord32 ptr_2, const SDouble ext_ident_double_230,
                              const SDouble ext_ownship_velocity_x, const SDouble *queue_7,
                              const SWord32 ptr_7, const SDouble ext_ident_double_231,
                              const SDouble ext_intruder_velocity_x, const SDouble *queue_3,
                              const SWord32 ptr_3, const SDouble ext_ident_double_232,
                              const SDouble ext_ownship_position_y, const SDouble *queue_4,
                              const SWord32 ptr_4, const SDouble ext_ident_double_233,
                              const SDouble ext_intruder_position_y, const SDouble *queue_5,
                              const SWord32 ptr_5, const SDouble ext_ident_double_234,
                              const SDouble ext_ownship_velocity_y, const SDouble *queue_8,
                              const SWord32 ptr_8, const SDouble ext_ident_double_235,
                              const SDouble ext_intruder_velocity_y, const SDouble *queue_6,
                              const SWord32 ptr_6, const SDouble ext_ident_double_293,
                              const SDouble ext_ident_double_251, const SBool ext_ident_bool_246,
                              const SDouble ext_ident_double_245,
                              const SDouble ext_ident_double_240,
                              const SDouble ext_ident_double_238,
                              const SDouble ext_ownship_position_z, const SDouble *queue_9,
                              const SWord32 ptr_9, const SDouble ext_ident_double_239,
                              const SDouble ext_intruder_position_z, const SDouble *queue_10,
                              const SWord32 ptr_10, const SDouble ext_ident_double_244,
                              const SDouble ext_ident_double_243,
                              const SDouble ext_ident_double_241,
                              const SDouble ext_ident_double_242,
                              const SDouble ext_minimal_vertical_separation,
                              const SDouble ext_ident_double_250,
                              const SDouble ext_direction_parameter_vertical,
                              const SDouble ext_ident_double_249,
                              const SDouble ext_ident_double_247,
                              const SDouble ext_ident_double_248,
                              const SDouble ext_ident_double_292, const SDouble ext_sqrt_291,
                              const SDouble ext_ident_double_260,
                              const SDouble ext_ident_double_252,
                              const SDouble ext_ident_double_253,
                              const SDouble ext_ident_double_254,
                              const SDouble ext_ident_double_255,
                              const SDouble ext_ident_double_256,
                              const SDouble ext_ident_double_257,
                              const SDouble ext_ident_double_258,
                              const SDouble ext_ident_double_259,
                              const SDouble ext_ident_double_269,
                              const SDouble ext_ident_double_261,
                              const SDouble ext_ident_double_262,
                              const SDouble ext_ident_double_263,
                              const SDouble ext_ident_double_264,
                              const SDouble ext_ident_double_265,
                              const SDouble ext_ident_double_266,
                              const SDouble ext_ident_double_267,
                              const SDouble ext_ident_double_268,
                              const SDouble ext_ident_double_290,
                              const SDouble ext_ident_double_279,
                              const SDouble ext_ident_double_278,
                              const SDouble ext_ident_double_270,
                              const SDouble ext_ident_double_271,
                              const SDouble ext_ident_double_272,
                              const SDouble ext_ident_double_273,
                              const SDouble ext_ident_double_274,
                              const SDouble ext_ident_double_275,
                              const SDouble ext_ident_double_276,
                              const SDouble ext_ident_double_277,
                              const SDouble ext_ident_double_289,
                              const SDouble ext_ident_double_288,
                              const SDouble ext_ident_double_280,
                              const SDouble ext_ident_double_281,
                              const SDouble ext_ident_double_282,
                              const SDouble ext_ident_double_283,
                              const SDouble ext_ident_double_284,
                              const SDouble ext_ident_double_285,
                              const SDouble ext_ident_double_286,
                              const SDouble ext_ident_double_287,
                              const SDouble ext_minimal_horizontal_separation,
                              const SDouble ext_ident_double_304,
                              const SDouble ext_ident_double_303,
                              const SDouble ext_ident_double_295,
                              const SDouble ext_ident_double_296,
                              const SDouble ext_ident_double_297,
                              const SDouble ext_ident_double_298,
                              const SDouble ext_ident_double_299,
                              const SDouble ext_ident_double_300,
                              const SDouble ext_ident_double_301,
                              const SDouble ext_ident_double_302)
{
  const SDouble s0 = ext_ident_double_306;
  const SDouble s1 = ext_ident_double_305;
  const SDouble s2 = ext_ident_double_294;
  const SDouble s3 = ext_ident_double_237;
  const SDouble s4 = ext_ident_double_236;
  const SDouble s5 = ext_ident_double_228;
  const SBool   s6 = queue_0[0];
  const SWord32 s7 = ptr_0;
  const SDouble s8 = ext_ownship_position_x;
  const SDouble s9 = queue_1[0];
  const SWord32 s10 = ptr_1;
  const SDouble s11 = ext_ident_double_229;
  const SDouble s12 = ext_intruder_position_x;
  const SDouble s13 = queue_2[0];
  const SWord32 s14 = ptr_2;
  const SDouble s15 = ext_ident_double_230;
  const SDouble s16 = ext_ownship_velocity_x;
  const SDouble s17 = queue_7[0];
  const SWord32 s18 = ptr_7;
  const SDouble s19 = ext_ident_double_231;
  const SDouble s20 = ext_intruder_velocity_x;
  const SDouble s21 = queue_3[0];
  const SWord32 s22 = ptr_3;
  const SDouble s23 = ext_ident_double_232;
  const SDouble s24 = ext_ownship_position_y;
  const SDouble s25 = queue_4[0];
  const SWord32 s26 = ptr_4;
  const SDouble s27 = ext_ident_double_233;
  const SDouble s28 = ext_intruder_position_y;
  const SDouble s29 = queue_5[0];
  const SWord32 s30 = ptr_5;
  const SDouble s31 = ext_ident_double_234;
  const SDouble s32 = ext_ownship_velocity_y;
  const SDouble s33 = queue_8[0];
  const SWord32 s34 = ptr_8;
  const SDouble s35 = ext_ident_double_235;
  const SDouble s36 = ext_intruder_velocity_y;
  const SDouble s37 = queue_6[0];
  const SWord32 s38 = ptr_6;
  const SDouble s39 = ext_ident_double_293;
  const SDouble s40 = ext_ident_double_251;
  const SBool   s41 = ext_ident_bool_246;
  const SDouble s42 = ext_ident_double_245;
  const SDouble s43 = ext_ident_double_240;
  const SDouble s44 = ext_ident_double_238;
  const SDouble s45 = ext_ownship_position_z;
  const SDouble s46 = queue_9[0];
  const SWord32 s47 = ptr_9;
  const SDouble s48 = ext_ident_double_239;
  const SDouble s49 = ext_intruder_position_z;
  const SDouble s50 = queue_10[0];
  const SWord32 s51 = ptr_10;
  const SDouble s52 = ext_ident_double_244;
  const SDouble s53 = ext_ident_double_243;
  const SDouble s54 = ext_ident_double_241;
  const SDouble s55 = ext_ident_double_242;
  const SDouble s56 = ext_minimal_vertical_separation;
  const SDouble s57 = ext_ident_double_250;
  const SDouble s58 = ext_direction_parameter_vertical;
  const SDouble s59 = ext_ident_double_249;
  const SDouble s60 = ext_ident_double_247;
  const SDouble s61 = ext_ident_double_248;
  const SDouble s62 = ext_ident_double_292;
  const SDouble s63 = ext_sqrt_291;
  const SDouble s64 = ext_ident_double_260;
  const SDouble s65 = ext_ident_double_252;
  const SDouble s66 = ext_ident_double_253;
  const SDouble s67 = ext_ident_double_254;
  const SDouble s68 = ext_ident_double_255;
  const SDouble s69 = ext_ident_double_256;
  const SDouble s70 = ext_ident_double_257;
  const SDouble s71 = ext_ident_double_258;
  const SDouble s72 = ext_ident_double_259;
  const SDouble s73 = ext_ident_double_269;
  const SDouble s74 = ext_ident_double_261;
  const SDouble s75 = ext_ident_double_262;
  const SDouble s76 = ext_ident_double_263;
  const SDouble s77 = ext_ident_double_264;
  const SDouble s78 = ext_ident_double_265;
  const SDouble s79 = ext_ident_double_266;
  const SDouble s80 = ext_ident_double_267;
  const SDouble s81 = ext_ident_double_268;
  const SDouble s82 = ext_ident_double_290;
  const SDouble s83 = ext_ident_double_279;
  const SDouble s84 = ext_ident_double_278;
  const SDouble s85 = ext_ident_double_270;
  const SDouble s86 = ext_ident_double_271;
  const SDouble s87 = ext_ident_double_272;
  const SDouble s88 = ext_ident_double_273;
  const SDouble s89 = ext_ident_double_274;
  const SDouble s90 = ext_ident_double_275;
  const SDouble s91 = ext_ident_double_276;
  const SDouble s92 = ext_ident_double_277;
  const SDouble s93 = ext_ident_double_289;
  const SDouble s94 = ext_ident_double_288;
  const SDouble s95 = ext_ident_double_280;
  const SDouble s96 = ext_ident_double_281;
  const SDouble s97 = ext_ident_double_282;
  const SDouble s98 = ext_ident_double_283;
  const SDouble s99 = ext_ident_double_284;
  const SDouble s100 = ext_ident_double_285;
  const SDouble s101 = ext_ident_double_286;
  const SDouble s102 = ext_ident_double_287;
  const SDouble s103 = ext_minimal_horizontal_separation;
  const SDouble s104 = ext_ident_double_304;
  const SDouble s105 = ext_ident_double_303;
  const SDouble s106 = ext_ident_double_295;
  const SDouble s107 = ext_ident_double_296;
  const SDouble s108 = ext_ident_double_297;
  const SDouble s109 = ext_ident_double_298;
  const SDouble s110 = ext_ident_double_299;
  const SDouble s111 = ext_ident_double_300;
  const SDouble s112 = ext_ident_double_301;
  const SDouble s113 = ext_ident_double_302;
  const SBool   s115 = s0 > 0.0;
  const SBool   s116 = s115 /* verticalCriterionConflict_part2.2 */;

  return s116;
}
