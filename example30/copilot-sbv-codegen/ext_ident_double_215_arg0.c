/* File: "ext_ident_double_215_arg0.c". Automatically generated by SBV. Do not edit! */
...
/*@
 assigns \nothing;
 ensures \result == ((( (ext_ident_bool_212) ? (ext_ident_double_214) : (-1.0))));
*/
...
{
  const SBool   s0 = ext_ident_bool_212;
  const SDouble s1 = ext_ident_double_211;
  const SDouble s2 = ext_ident_double_208;
  const SDouble s3 = ext_ownship_position_z;
  const SDouble s4 = ext_intruder_position_z;
  const SDouble s5 = ext_ident_double_210;
  const SDouble s6 = ext_ident_double_209;
  const SDouble s7 = ext_minimal_vertical_separation;
  const SDouble s8 = ext_ident_double_214;
