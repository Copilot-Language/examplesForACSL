...
/*@
 assigns \nothing;
 ensures \result == ((( (ext_ident_bool_105) ? (ext_ident_double_107) : (-1.0))));
*/
...
{
  const SBool   s0 = ext_ident_bool_105;
  const SDouble s1 = ext_ident_double_104;
  const SDouble s2 = ext_ident_double_101;
  const SDouble s3 = ext_ownship_position_z;
  const SDouble s4 = ext_intruder_position_z;
  const SDouble s5 = ext_ident_double_103;
