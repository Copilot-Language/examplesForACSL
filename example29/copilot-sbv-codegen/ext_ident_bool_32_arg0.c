/* File: "ext_ident_bool_32_arg0.c". Automatically generated by SBV. Do not edit! */

#include "internal.h"

/* User given declarations: */
/*test 003*/
/*ACSL to write
 label "verticalCriterionConflict_part1" (Extf_ident_bool(label "verticalCriterionConflict_part1.1" (Extf_ident_bool(label "verticalCriterionConflict_part1.1.1" (Extf_ident_double(label "normsq2dim" (((Ext_ownship_position_x - Ext_intruder_position_x) * (Ext_ownship_position_x - Ext_intruder_position_x)) + ((Ext_ownship_position_y - Ext_intruder_position_y) * (Ext_ownship_position_y - Ext_intruder_position_y)))) <= 0.0)) && Extf_ident_bool(label "verticalCriterionConflict_part1.1.2" (Extf_ident_double(label "normsq2dim" (((Ext_ownship_position_x - Ext_intruder_position_x) * (Ext_ownship_position_x - Ext_intruder_position_x)) + ((Ext_ownship_position_y - Ext_intruder_position_y) * (Ext_ownship_position_y - Ext_intruder_position_y)))) >= 0.0)))) && (Extf_ident_bool(label "verticalCriterionConflict_part1.2" ((Ext_ownship_planned_velocity_z - Ext_intruder_velocity_z) >= 0.0)) && Extf_ident_bool(label "verticalCriterionConflict_part1.3" ((Ext_direction_parameter_vertical * (Ext_ownship_position_z - Ext_intruder_position_z)) >= Ext_minimal_vertical_separation))))
*/
/*@
 assigns \nothing;
 ensures \result == ((((ext_ident_bool_29) && (((ext_ident_bool_30) && (ext_ident_bool_31))))));
*/
SBool ext_ident_bool_32_arg0(const SBool ext_ident_bool_29,
                             const SBool ext_ident_bool_26, const SDouble ext_ident_double_25,
                             const SDouble ext_ownship_position_x,
                             const SDouble ext_intruder_position_x,
                             const SDouble ext_ownship_position_y,
                             const SDouble ext_intruder_position_y,
                             const SBool ext_ident_bool_28, const SDouble ext_ident_double_27,
                             const SBool ext_ident_bool_30,
                             const SDouble ext_ownship_planned_velocity_z,
                             const SDouble ext_intruder_velocity_z,
                             const SBool ext_ident_bool_31,
                             const SDouble ext_direction_parameter_vertical,
                             const SDouble ext_ownship_position_z,
                             const SDouble ext_intruder_position_z,
                             const SDouble ext_minimal_vertical_separation)
{
  const SBool   s0 = ext_ident_bool_29;
  const SBool   s1 = ext_ident_bool_26;
  const SDouble s2 = ext_ident_double_25;
  const SDouble s3 = ext_ownship_position_x;
  const SDouble s4 = ext_intruder_position_x;
  const SDouble s5 = ext_ownship_position_y;
  const SDouble s6 = ext_intruder_position_y;
  const SBool   s7 = ext_ident_bool_28;
  const SDouble s8 = ext_ident_double_27;
  const SBool   s9 = ext_ident_bool_30;
  const SDouble s10 = ext_ownship_planned_velocity_z;
  const SDouble s11 = ext_intruder_velocity_z;
  const SBool   s12 = ext_ident_bool_31;
  const SDouble s13 = ext_direction_parameter_vertical;
  const SDouble s14 = ext_ownship_position_z;
  const SDouble s15 = ext_intruder_position_z;
  const SDouble s16 = ext_minimal_vertical_separation;
  const SBool   s17 = s9 && s12;
  const SBool   s18 = s0 && s17;
  const SBool   s19 = s18 /* verticalCriterionConflict_part1 */;

  return s19;
}
