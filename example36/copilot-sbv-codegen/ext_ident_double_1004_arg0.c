/* File: "ext_ident_double_1004_arg0.c". Automatically generated by SBV. Do not edit! */

#include "internal.h"

/* User given declarations: */
/*test 003*/
/*DotBegin
digraph G {
node [shape=box]

0 [label="file: 
?????",color=red, style=filled]
1 [label="label: mini",color=plum, style=filled]
0 -> 1
2 [label="op3: mux",color=green4, style=filled]
1 -> 2
3 [label="op2: <=",color=green4, style=filled]
2 -> 3
4 [label="ext_ident_double_935",color=cyan4, style=filled]
3 -> 4
648 [label="ext_ident_double_958",color=cyan4, style=filled]
3 -> 648
1307 [label="ext_ident_double_980",color=cyan4, style=filled]
2 -> 1307
1951 [label="ext_ident_double_1003",color=cyan4, style=filled]
2 -> 1951


}

DotEnd*/
/*@
 assigns \nothing;
 ensures \result == ((( (((ext_ident_double_935) <= (ext_ident_double_958))) ? (ext_ident_double_980) : (ext_ident_double_1003))));
*/
SDouble ext_ident_double_1004_arg0(const SDouble ext_ident_double_935,
                                   const SDouble ext_ident_double_914,
                                   const SDouble ext_ownship_position_x,
                                   const SDouble ext_intruder_position_x,
                                   const SDouble ext_ownship_velocity_x,
                                   const SDouble ext_intruder_velocity_x,
                                   const SDouble ext_ownship_position_y,
                                   const SDouble ext_intruder_position_y,
                                   const SDouble ext_ownship_velocity_y,
                                   const SDouble ext_intruder_velocity_y,
                                   const SDouble ext_ident_double_931,
                                   const SDouble ext_ident_double_922,
                                   const SWord64 ext_ident_word64_915,
                                   const SDouble ext_ownship_position_z,
                                   const SWord64 ext_ident_word64_916,
                                   const SWord64 ext_ident_word64_917,
                                   const SWord64 ext_ident_word64_918,
                                   const SWord64 ext_ident_word64_919,
                                   const SWord64 ext_ident_word64_920,
                                   const SWord64 ext_ident_word64_921,
                                   const SDouble ext_ident_double_930,
                                   const SWord64 ext_ident_word64_923,
                                   const SWord64 ext_ident_word64_924,
                                   const SWord64 ext_ident_word64_925,
                                   const SWord64 ext_ident_word64_926,
                                   const SWord64 ext_ident_word64_927,
                                   const SWord64 ext_ident_word64_928,
                                   const SWord64 ext_ident_word64_929,
                                   const SDouble ext_ident_double_933,
                                   const SDouble ext_ident_double_932,
                                   const SDouble ext_ident_double_934,
                                   const SDouble ext_ident_double_958,
                                   const SDouble ext_ident_double_957,
                                   const SDouble ext_ident_double_936,
                                   const SDouble ext_ident_double_953,
                                   const SDouble ext_ident_double_944,
                                   const SWord64 ext_ident_word64_937,
                                   const SWord64 ext_ident_word64_938,
                                   const SWord64 ext_ident_word64_939,
                                   const SWord64 ext_ident_word64_940,
                                   const SWord64 ext_ident_word64_941,
                                   const SWord64 ext_ident_word64_942,
                                   const SWord64 ext_ident_word64_943,
                                   const SDouble ext_ident_double_952,
                                   const SWord64 ext_ident_word64_945,
                                   const SWord64 ext_ident_word64_946,
                                   const SWord64 ext_ident_word64_947,
                                   const SWord64 ext_ident_word64_948,
                                   const SWord64 ext_ident_word64_949,
                                   const SWord64 ext_ident_word64_950,
                                   const SWord64 ext_ident_word64_951,
                                   const SDouble ext_ident_double_955,
                                   const SDouble ext_ident_double_954,
                                   const SDouble ext_ident_double_956,
                                   const SDouble ext_ownship_velocity_z,
                                   const SDouble ext_ident_double_980,
                                   const SDouble ext_ident_double_959,
                                   const SDouble ext_ident_double_976,
                                   const SDouble ext_ident_double_967,
                                   const SWord64 ext_ident_word64_960,
                                   const SWord64 ext_ident_word64_961,
                                   const SWord64 ext_ident_word64_962,
                                   const SWord64 ext_ident_word64_963,
                                   const SWord64 ext_ident_word64_964,
                                   const SWord64 ext_ident_word64_965,
                                   const SWord64 ext_ident_word64_966,
                                   const SDouble ext_ident_double_975,
                                   const SWord64 ext_ident_word64_968,
                                   const SWord64 ext_ident_word64_969,
                                   const SWord64 ext_ident_word64_970,
                                   const SWord64 ext_ident_word64_971,
                                   const SWord64 ext_ident_word64_972,
                                   const SWord64 ext_ident_word64_973,
                                   const SWord64 ext_ident_word64_974,
                                   const SDouble ext_ident_double_978,
                                   const SDouble ext_ident_double_977,
                                   const SDouble ext_ident_double_979,
                                   const SDouble ext_ident_double_1003,
                                   const SDouble ext_ident_double_1002,
                                   const SDouble ext_ident_double_981,
                                   const SDouble ext_ident_double_998,
                                   const SDouble ext_ident_double_989,
                                   const SWord64 ext_ident_word64_982,
                                   const SWord64 ext_ident_word64_983,
                                   const SWord64 ext_ident_word64_984,
                                   const SWord64 ext_ident_word64_985,
                                   const SWord64 ext_ident_word64_986,
                                   const SWord64 ext_ident_word64_987,
                                   const SWord64 ext_ident_word64_988,
                                   const SDouble ext_ident_double_997,
                                   const SWord64 ext_ident_word64_990,
                                   const SWord64 ext_ident_word64_991,
                                   const SWord64 ext_ident_word64_992,
                                   const SWord64 ext_ident_word64_993,
                                   const SWord64 ext_ident_word64_994,
                                   const SWord64 ext_ident_word64_995,
                                   const SWord64 ext_ident_word64_996,
                                   const SDouble ext_ident_double_1000,
                                   const SDouble ext_ident_double_999,
                                   const SDouble ext_ident_double_1001)
{
  const SDouble s0 = ext_ident_double_935;
  const SDouble s1 = ext_ident_double_914;
  const SDouble s2 = ext_ownship_position_x;
  const SDouble s3 = ext_intruder_position_x;
  const SDouble s4 = ext_ownship_velocity_x;
  const SDouble s5 = ext_intruder_velocity_x;
  const SDouble s6 = ext_ownship_position_y;
  const SDouble s7 = ext_intruder_position_y;
  const SDouble s8 = ext_ownship_velocity_y;
  const SDouble s9 = ext_intruder_velocity_y;
  const SDouble s10 = ext_ident_double_931;
  const SDouble s11 = ext_ident_double_922;
  const SWord64 s12 = ext_ident_word64_915;
  const SDouble s13 = ext_ownship_position_z;
  const SWord64 s14 = ext_ident_word64_916;
  const SWord64 s15 = ext_ident_word64_917;
  const SWord64 s16 = ext_ident_word64_918;
  const SWord64 s17 = ext_ident_word64_919;
  const SWord64 s18 = ext_ident_word64_920;
  const SWord64 s19 = ext_ident_word64_921;
  const SDouble s20 = ext_ident_double_930;
  const SWord64 s21 = ext_ident_word64_923;
  const SWord64 s22 = ext_ident_word64_924;
  const SWord64 s23 = ext_ident_word64_925;
  const SWord64 s24 = ext_ident_word64_926;
  const SWord64 s25 = ext_ident_word64_927;
  const SWord64 s26 = ext_ident_word64_928;
  const SWord64 s27 = ext_ident_word64_929;
  const SDouble s28 = ext_ident_double_933;
  const SDouble s29 = ext_ident_double_932;
  const SDouble s30 = ext_ident_double_934;
  const SDouble s31 = ext_ident_double_958;
  const SDouble s32 = ext_ident_double_957;
  const SDouble s33 = ext_ident_double_936;
  const SDouble s34 = ext_ident_double_953;
  const SDouble s35 = ext_ident_double_944;
  const SWord64 s36 = ext_ident_word64_937;
  const SWord64 s37 = ext_ident_word64_938;
  const SWord64 s38 = ext_ident_word64_939;
  const SWord64 s39 = ext_ident_word64_940;
  const SWord64 s40 = ext_ident_word64_941;
  const SWord64 s41 = ext_ident_word64_942;
  const SWord64 s42 = ext_ident_word64_943;
  const SDouble s43 = ext_ident_double_952;
  const SWord64 s44 = ext_ident_word64_945;
  const SWord64 s45 = ext_ident_word64_946;
  const SWord64 s46 = ext_ident_word64_947;
  const SWord64 s47 = ext_ident_word64_948;
  const SWord64 s48 = ext_ident_word64_949;
  const SWord64 s49 = ext_ident_word64_950;
  const SWord64 s50 = ext_ident_word64_951;
  const SDouble s51 = ext_ident_double_955;
  const SDouble s52 = ext_ident_double_954;
  const SDouble s53 = ext_ident_double_956;
  const SDouble s54 = ext_ownship_velocity_z;
  const SDouble s55 = ext_ident_double_980;
  const SDouble s56 = ext_ident_double_959;
  const SDouble s57 = ext_ident_double_976;
  const SDouble s58 = ext_ident_double_967;
  const SWord64 s59 = ext_ident_word64_960;
  const SWord64 s60 = ext_ident_word64_961;
  const SWord64 s61 = ext_ident_word64_962;
  const SWord64 s62 = ext_ident_word64_963;
  const SWord64 s63 = ext_ident_word64_964;
  const SWord64 s64 = ext_ident_word64_965;
  const SWord64 s65 = ext_ident_word64_966;
  const SDouble s66 = ext_ident_double_975;
  const SWord64 s67 = ext_ident_word64_968;
  const SWord64 s68 = ext_ident_word64_969;
  const SWord64 s69 = ext_ident_word64_970;
  const SWord64 s70 = ext_ident_word64_971;
  const SWord64 s71 = ext_ident_word64_972;
  const SWord64 s72 = ext_ident_word64_973;
  const SWord64 s73 = ext_ident_word64_974;
  const SDouble s74 = ext_ident_double_978;
  const SDouble s75 = ext_ident_double_977;
  const SDouble s76 = ext_ident_double_979;
  const SDouble s77 = ext_ident_double_1003;
  const SDouble s78 = ext_ident_double_1002;
  const SDouble s79 = ext_ident_double_981;
  const SDouble s80 = ext_ident_double_998;
  const SDouble s81 = ext_ident_double_989;
  const SWord64 s82 = ext_ident_word64_982;
  const SWord64 s83 = ext_ident_word64_983;
  const SWord64 s84 = ext_ident_word64_984;
  const SWord64 s85 = ext_ident_word64_985;
  const SWord64 s86 = ext_ident_word64_986;
  const SWord64 s87 = ext_ident_word64_987;
  const SWord64 s88 = ext_ident_word64_988;
  const SDouble s89 = ext_ident_double_997;
  const SWord64 s90 = ext_ident_word64_990;
  const SWord64 s91 = ext_ident_word64_991;
  const SWord64 s92 = ext_ident_word64_992;
  const SWord64 s93 = ext_ident_word64_993;
  const SWord64 s94 = ext_ident_word64_994;
  const SWord64 s95 = ext_ident_word64_995;
  const SWord64 s96 = ext_ident_word64_996;
  const SDouble s97 = ext_ident_double_1000;
  const SDouble s98 = ext_ident_double_999;
  const SDouble s99 = ext_ident_double_1001;
  const SBool   s100 = s0 <= s31;
  const SDouble s101 = s100 ? s55 : s77;
  const SDouble s102 = s101 /* mini */;

  return s102;
}
