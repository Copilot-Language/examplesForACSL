...
/*test 001*/
/*ACSL to write
 (if s1 then s2 else (if s6 then s7 else s0))
*/
/*@
 assigns \nothing;
 ensures \result == (( (queue_1[ptr_1]) ? (queue_2[ptr_2]) : (( (queue_6[ptr_6]) ? (queue_7[ptr_7]) : (queue_0[ptr_0])))));
*/
                       ...
                       const SWord16 *queue_0, const SWord32 ptr_0)
{
  ...
  const SWord16 s21 = queue_7[0];
  const SWord32 s22 = ptr_7;
  const SWord16 s23 = queue_0[0];
  const SWord32 s24 = ptr_0;
  const SBool table0[] = {
