...
/*test 001*/
/*ACSL to write
 (if s1 then s3 else (if s6 then s0 else s2))
*/
/*@
 assigns \nothing;
 ensures \result == (( (queue_1[ptr_1]) ? (queue_3[ptr_3]) : (( (queue_6[ptr_6]) ? (queue_0[ptr_0]) : (queue_2[ptr_2])))));
*/
                       ...
                       const SWord16 *queue_2, const SWord32 ptr_2)
{
  ...
  const SWord16 s21 = queue_0[0];
  const SWord32 s22 = ptr_0;
  const SWord16 s23 = queue_2[0];
  const SWord32 s24 = ptr_2;
  const SBool table0[] = {
